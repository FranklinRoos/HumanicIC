...
function overzicht()
{
    $objecten = mysql_query("SELECT * FROM nav") or die(mysql_error());
...
function navBewerken()
{
    ...
    $bericht = mysql_query("SELECT * FROM nav WHERE nav_id = ".$_GET['nav_id']." LIMIT 1") or die(mysql_error());
...
{   
    ...
    {
        mysql_query("UPDATE `nav` SET `nav_id` ='".$_POST['nav_id']."', `nav_naam` ='".$nav_naam."', `nav_place`='".$nav_place."', `nav_show`='".$nav_show."' WHERE `nav_id` = ".$_POST['nav_id']." ") or die(mysql_error());
    }
...
{
    $result_sql = mysql_query("SELECT * FROM nav WHERE nav_id=".$result_id."");
